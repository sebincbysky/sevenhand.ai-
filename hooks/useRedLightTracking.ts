...
export const useRedLightTracking = (webcamRef: React.RefObject<Webcam | null>, isEnabled: boolean = true) => {
      ...
      if (video.readyState >= 2 && !video.paused && !video.ended) {
        ...
        if (ctx) {
          ...
          for (let i = 0; i < data.length; i += 4) {
            ...
            // Increased strictness for red dominance to avoid false positives
            if (r > 150 && r > g * 2 && r > b * 2 && g < 100 && b < 100) {
          ...
          // Lowered count to 2 to detect smaller laser pointers
          if (count > 2) { 
